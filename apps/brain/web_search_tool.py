...
# ---------------------------------------------------------------------------
# A single AsyncClient is reused across all requests for connection pooling,
# reducing TCP handshake overhead significantly on repeated searches.
# NOTE: verify=True (default) — TLS certificates are validated. If you are
# behind a corporate proxy/AV with custom root CA, set the REQUESTS_CA_BUNDLE
# or SSL_CERT_FILE environment variable to point to your CA bundle instead of
...
async def close_client() -> None:
    """Close the shared client on app shutdown (call from lifespan)."""
    global _SHARED_CLIENT
...
async def fetch_url(url: str, timeout: int = 15, **kwargs: Any) -> str:
    ...
    try:
        client = _get_client()
        r = await client.get(clean_url, timeout=timeout)
