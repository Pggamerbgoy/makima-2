...
5. **Mandatory Telemetry, SDK Hooks & Outage Triage Protocols**:
   - **Lifecycle Event vs Loop Disambiguation**: When debugging suspected model loops or high tool call counts in benchmarks/tests, **never** rely on raw event counts without checking event types. Verify whether the test harness interceptor is counting lifecycle events (`started`, `finished`, `completed`, `toast`) as separate tool calls. Interceptors must strictly filter for *invocation start* events.
   - **SDK Hook Context & Signature Verification**: Never assume hook signatures or parameter structures across external SDKs (e.g. OpenAI Agents SDK). In OpenAI Agents SDK, tool arguments reside inside `context.tool_arguments` or `context.tool_call.arguments`, not on an ad-hoc `call` parameter. Any loop detector or cache must verify that argument extraction succeeded before computing fingerprints, and must never treat empty extraction as proof of identical repeated calls.
   - **Catastrophic Regression & Outage Triage**: If an automated suite or benchmark experiences an abrupt, catastrophic metric collapse (e.g. 75% -> 15%), **immediately check provider HTTP status codes and quota errors before assuming a code bug**. Explicitly verify API connectivity (`HTTP 403 Quota`, `HTTP 429 RateLimit`, `HTTP 401 Auth`, `HTTP 500 Outage`) before attempting to revert or rewrite recent logic changes.